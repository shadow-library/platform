...
- **Server-sent events:** `EventStream.open(reply, { heartbeatMs?, maxLifetimeMs?, retryMs? })` in a route that takes `@Res()` — hijacks the reply (so `onSend` hooks such as compression never buffer it), keeps headers already set on the reply, writes `retry`, heartbeats (default 20s) and ends itself after `maxLifetimeMs` (default 15m) so the client reconnects through the route's guards. `.send({ event?, data, id? })` (returns `false` once closed), `.onClose(fn)`, `.close()`, `.closed`. `FastifyRouter.stop()` calls `EventStream.closeAll(instance)` first — a connected stream would otherwise hold Fastify's `close()` open forever.
...
Exports: `AuthClient` (incl. `resolveBotKey(key, clientIp?)`), bot keys (`BOT_KEY_PREFIX`, `parseBotKey`, `isBotKeyShaped`, `formatBotKey`, `botKeyIdToUuid`, `ParsedBotKey`), `BotKeyExchanger` (+ `BotKeyExchangerOptions`), `BotRateLimiter` (+ `BotRateLimiterOptions`, `BotRateDecision`), `assertValidRoleCatalog`, `AppSessionClient` (+ `AppSessionClientOptions`), `AppRegistryClient` (+ `AppRegistryOptions`), `ServiceAccessClient` (+ `ServiceAccessOptions`), `AccessTokenCache` (+ `AccessTokenKey`), `hashSessionHandle`, `AuthErrorCode`, `decodeJwt`, `validateClaims`, `verifyJwt`, and types `ClaimExpectations`, `DecodedJwt`, `JwtHeader`, `Jwk`, `JwtPayload`, `PrincipalKind`, `AssuranceLevel`, `AuthPrincipal` (union of `UserPrincipal` / `ServicePrincipal` / `BotPrincipal`), `FetchLike`, `AuthClientCredential`, `AuthCacheOptions`, `PermissionManifest`, `RoleManifest`, `BotGrantManifest`, `BotGrantLevel`, `RoleCatalogManifest`, `RoleCatalogSyncResult`, `RoleCatalogSyncOptions`, `AuthClientConfig`, `ServiceAccessRule`, `DiscoveryDocument`, `ServiceTokenOptions`, `CheckPrincipal`, `CheckInput`, `CheckOptions`, `IntrospectionResult`, `LogoutTokenClaims`, `AppSession`, `AppSessionCreateInput`, `AppSessionTokenInput`, `AppSessionToken`, `AppSessionElevation`, `AppSessionOrganisation`, `SwitchedOrganisation`, `AppRegistration`, `TokenExchangeInput`, `ExchangedToken`.
...
- **Decorators are `@Authenticated()`, `@RequireScope(...scopes)`, `@RequirePermission(permission, options?)`, `@RequireElevation(...scopes)`, `@BotPermission(permission)`** — there is **no `@Auth` and no `@Public`** in the SDK (confirmed absent). They work class- or method-level; class-level metadata deep-merges into each handler (via app's handler-metadata merge).
- **Protection is opt-in, not default-deny:** `AuthGuard` (a `@Middleware({type:'preValidation', weight:100})` generating class) skips routes without auth metadata. An app wanting default-deny + a `@Public()` escape hatch builds that in its own auth module on top (some apps do — follow the repo).
- **The guard runs a stage ahead of schema validation.** `preValidation` precedes Fastify's validation stage, so an unauthenticated caller gets a 401 rather than a 422 describing the route's body schema. A guard of your own that reads params, the body, or the principal belongs on `preHandler`, which still runs strictly after.
...
- A guarded route resolves its principal from **either** an `Authorization: Bearer` token **or** the app-session cookie, into one `AuthPrincipal`. M2M/service principals are deny-by-default (must match an admin-loaded `ServiceAccessRule`); PDP `check` fails closed unless `failOpen`. Bot principals (`kind: 'bot'`) enter only by exchanging an `sl_bot_…` key at identity — `verify()` refuses a bot token — and are refused, before any exchange, on every route without `@BotPermission` and on elevated routes; a bot must hold every `@BotPermission` plus the route's `@RequirePermission`. See `docs/identity.md`.
...
- Not every backend uses `run()`/`constraintErrorMap` — `web-novel-server` deliberately follows a direct-client pattern instead. Mirror the repo you're in.
