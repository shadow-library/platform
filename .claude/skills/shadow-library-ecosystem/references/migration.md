...
- Adopt the root tooling: **delete** the workspace's own `verify`/`build`/`check-migrations`/
  ...
  (`bun scripts/verify.ts <workspace>`, `bun scripts/build.ts <workspace>`, …). Also **delete**
  `type-check` (a `tsc` pass-through — `verify.ts` runs `tsc` directly against the workspace's own
  tsconfig), any `db:*` pointer script (`bun scripts/db.ts <workspace> <cmd>` from the repo root
  replaces it), `clean`, and a bare `"test": "bun test"` pass-through (`verify.ts` falls back to running
  `bun test` directly when the workspace declares none). Keep only what is genuinely the workspace's own
  — `dev`, and a `test` script only if it's not a plain `bun test` restatement (Playwright, `vitest`, a
  composed sequence). Remove any hand-rolled scripts/configs and any leftover `.shadowrc.json` (that
  format no longer exists — non-inferable build inputs go in a `"shadow"` key in the workspace's
  `package.json`). Do not add a workspace-local `.prettierrc.json`, `commitlint.config.*`, or
  `eslint.config.ts` — the root config covers every workspace; a lint deviation is a `files`-scoped block
  in the root `eslint.config.ts`, not a workspace-local config file. Per `references/repository-setup.md`,
  husky hooks live once at the repo root; don't wire per-workspace hooks.
...
has no router-owned SSR/QueryClient concern to migrate, just the router+query wiring; no current web app is
`type: spa` — all four are `ssr`)
...
When a workspace already uses _some_ of the ecosystem, look specifically for these gaps and close them:
