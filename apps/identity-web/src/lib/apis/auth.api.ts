...
export const consentPromptQueryOptions = (clientId: string, scope: string, enabled = true) =>
  queryOptions<ConsentPrompt, ApiError>({
    ...
    queryFn: ({ signal }) => APIRequest.get('/auth/consent').query({ clientId, scope }).signal(signal).execute<ConsentPrompt>(),
    retry: false,
...
export const authApi = {
  loginInit(identifier: string, deviceId?: string, returnTo?: string): Promise<FlowState> {
    return APIRequest.post('/auth/login/init').body({ identifier, deviceId, returnTo }).execute<FlowState>();
  },

  registerInit(email: string, deviceId?: string): Promise<FlowState> {
    return APIRequest.post('/auth/register/init').body({ email, deviceId }).execute<FlowState>();
  },
  ...
  registerProfile(flowId: string, firstName: string, lastName: string): Promise<FlowState> {
    return APIRequest.post('/auth/register/profile').body({ flowId, firstName, lastName }).execute<FlowState>();
  },

  registerPassword(flowId: string, password: string): Promise<FlowState> {
    return APIRequest.post('/auth/register/password').body({ flowId, password }).modeled(401).execute<FlowState>();
  },

  recoverInit(identifier: string, deviceId?: string): Promise<FlowState> {
    return APIRequest.post('/auth/recover/init').body({ identifier, deviceId }).execute<FlowState>();
  },

  recoverReset(flowId: string, newPassword: string): Promise<FlowState> {
    return APIRequest.post('/auth/recover/reset').body({ flowId, newPassword }).modeled(401).execute<FlowState>();
  },
  ...
  loginResetPassword(flowId: string, currentPassword: string, newPassword: string): Promise<FlowState> {
    return APIRequest.post('/auth/login/reset-password').body({ flowId, currentPassword, newPassword }).modeled(401).execute<FlowState>();
  },
  ...
  challengeChange(flowId: string, method: ChallengeMethodName): Promise<FlowState> {
    return APIRequest.post('/auth/challenge/change').body({ flowId, method }).execute<FlowState>();
  },

  challengeResend(flowId: string, method: 'EMAIL_OTP' | 'SMS_OTP'): Promise<ResendResult> {
    return APIRequest.post('/auth/challenge/resend').body({ flowId, method }).modeled(429).execute<ResendResult>();
  },

  webauthnOptions(flowId?: string, deviceId?: string): Promise<WebauthnChallenge> {
    return APIRequest.post('/auth/webauthn/options').body({ flowId, deviceId }).execute<WebauthnChallenge>();
  },

  cancelFlow(flowId: string): Promise<void> {
    return APIRequest.post('/auth/cancel').body({ flowId }).execute<undefined>();
  },

  consentPrompt(clientId: string, scope: string): Promise<ConsentPrompt> {
    return APIRequest.get('/auth/consent').query({ clientId, scope }).execute<ConsentPrompt>();
  },
...
export function useSignoutMutation(): UseMutationResult<undefined, ApiError, undefined> {
  ...
  return useMutation<undefined, ApiError, undefined>({
    mutationFn: () => APIRequest.post('/auth/signout').body({}).execute<undefined>(),
