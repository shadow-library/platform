...
export function useSearchParams(): SearchParamsApi {
  ...
  // stable (deps: [router]) without closing over a potentially stale `search`.
  const appendSearch = useCallback(
    (params: SearchInput) => router.navigate({ search: cleanParams({ ...(router.state.location.search as SearchParams), ...params }) as never }),
    [router],
  );
