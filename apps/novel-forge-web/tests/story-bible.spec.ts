...
describe('factCountsBySubject', () => {
  it('should count each fact once per subject it names', () => {
    const counts = factCountsBySubject([fact('oath', ['amara', 'boone']), fact('scar', ['amara']), fact('echo', ['boone', 'boone'])]);
    expect(counts.get('amara')).toBe(2);
    expect(counts.get('boone')).toBe(2);
  });

  it('should not count an entity that only learned a fact', () => {
    expect(factCountsBySubject([fact('tideglass-price', ['tideglass'], ['amara'])]).has('amara')).toBe(false);
  });
});

describe('factCountLabel', () => {
  it('should say there are no facts rather than showing zero', () => {
    expect(factCountLabel(0)).toBe('No facts');
  });

  it('should use the singular for exactly one and the plural otherwise', () => {
    expect(factCountLabel(1)).toBe('1 fact');
    expect(factCountLabel(4)).toBe('4 facts');
  });
});
