...
/**
 * memoir-server's error codes in the owner's words. The wire carries only `{ code, message }` and no
...
const ERROR_CATALOGUE: Record<string, ErrorEntry> = {
  ...
  API_REQUEST_TIMEOUT: { kind: 'unavailable', copy: 'Memoir took too long to answer.' },
  API_REQUEST_NETWORK_ERROR: { kind: 'unavailable', copy: 'Couldn’t reach Memoir.' },
  ...
  [NETWORK_ERROR_CODE]: { kind: 'unavailable', copy: 'Couldn’t reach Memoir.' },
};
