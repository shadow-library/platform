...
/**
 * The parent of both the app chrome and the bare setup layout, so crossing between them never remounts the sync engine or drops the gate's
 * setup latch. `useSessionGuard` flips to `redirecting` once the session is gone, so a signed-out owner never keeps seeing the app.
 */
...
const ACCOUNT_FAILURE_COPY: Record<AccountLoadFailedProps['reason'], AccountFailureCopy> = {
  server: {
    ...
    description: 'Memoir didn’t respond, so it can’t tell which screens are ready for you yet. Anything on this device is kept.',
  },
  offline: {
    screen: 'Offline',
    title: 'You’re offline',
    description: 'Memoir needs a connection to open your account on this device for the first time. It will as soon as you reconnect.',
  },
  'signed-out': {
