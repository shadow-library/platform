...
function stripCopy({ state, queuedCount, readiness }: SyncSnapshot): StripCopy {
  ...
  if (readiness.kind === 'ready') return { message: "Couldn't reach Memoir. Everything on this device is kept.", action: 'retry' };
  return { message: "Couldn't reach Memoir, so your data hasn't loaded yet.", action: 'retry' };
}
