...
describe('document titles', () => {
  ...
  const routes: [string, string][] = [
    ['/', 'Today · Memoir'],
    ['/plan', 'Planning Board · Memoir'],
    ['/quests', 'Quests · Memoir'],
    ['/quests/new', 'New quest · Memoir'],
    ['/quests/q1', 'Quest details · Memoir'],
    ['/quests/q1/edit', 'Edit quest · Memoir'],
    ['/log', 'Journal · Memoir'],
    ['/log/meals', 'Meals · Memoir'],
    ['/log/weight', 'Weight · Memoir'],
    ['/log/health', 'Body & health · Memoir'],
    ['/log/sidequests', 'Side quests · Memoir'],
    ['/finance', 'Money · Memoir'],
    ['/finance/expenses/e1', 'Expense · Memoir'],
    ['/finance/subscriptions', 'Subscriptions · Memoir'],
    ['/finance/categories', 'Categories · Memoir'],
    ['/history', 'History · Memoir'],
    ['/insights', 'Insights · Memoir'],
    ['/review', 'Weekly Review · Memoir'],
    ['/ai', 'Ask · Memoir'],
    ['/hero', 'Hero · Memoir'],
    ['/hero/recovery', 'Coming back · Memoir'],
    ['/settings', 'Settings · Memoir'],
    ['/settings/notifications', 'Notifications · Memoir'],
    ['/settings/billing', 'Plan and billing · Memoir'],
    ['/settings/export', 'Data export · Memoir'],
    ['/settings/delete', 'Delete your data · Memoir'],
    ['/settings/app', 'App and sync · Memoir'],
    ['/nope', 'Not found · Memoir'],
    ['/log/nope', 'Not found · Memoir'],
  ];
  ...
  it('should title the setup route', async () => {
    ...
    await expectTitleAt('/onboarding', 'Set up · Memoir');
  });
  ...
  it('should title a route whose guard failed as an error rather than the screen it never reached', async () => {
    ...
    await expectTitleAt('/finance', 'Couldn’t open this page · Memoir');
  });
  ...
  it('should title a route whose guard was refused access as no access rather than a failed load', async () => {
    ...
    await expectTitleAt('/finance', 'This account has no access · Memoir');
  });
  ...
  it('should replace a loaded screen title when its guard fails on a later load', async () => {
    ...
    await waitFor(() => expect(document.title).toBe('Couldn’t open this page · Memoir'));
  });
});
