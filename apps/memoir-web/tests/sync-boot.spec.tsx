...
describe('NetStrip first sync', () => {
  ...
  it('should not mention queued changes when nothing is queued', async () => {
    ...
    await waitFor(() => expect(screen.getByRole('status').textContent).toMatch(/couldn't reach memoir, so your data hasn't loaded/i));
  ...
  it('should say a failed sync over a pulled mirror keeps the device data', async () => {
    ...
    await waitFor(() => expect(screen.getByRole('status').textContent).toMatch(/couldn't reach memoir\. everything on this device is kept/i));
  });
