...
describe.if(pgAvailable)('RefineService', () => {
  ...
  beforeAll(async () => {
    ...
    const modelRouter = new ModelRouterService(noop, databaseService, { enforce: async () => undefined } as never, { defaultsFor: async () => undefined } as never);
  ...
  it('previews context packs for the refinement purposes without touching a model', async () => {
    const chat = await refine.previewContext(projectId, { purpose: 'chat', scopeType: 'volume' });
