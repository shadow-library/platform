...
- `docs/plan-import-design.md` — offline plan authoring: the JSON bundle contract and transactional `plan/import` endpoint with overwrite/approve semantics, retained for
  legacy bundles from the now-retired `novel-plan-forge` skill; in-app authoring (hub bootstrap interview + chat ops + facts panel) is the primary path today; drives
  tasks PI1–PI3.
...
- `docs/reforge-transform-design.md` — the structural re-authoring mode: a windowed source analysis, a human-approved span → output-chapter plan, an append-only cut ledger, an
  N:M transform write, and promotion into a publishable project. **Adds Appendix A rules 16–17** (its §2); drives tasks RT1–RT11. The 1:1 reforge path and chapter generation are
  untouched — transform is gated on `reforges.mode`.
...
- `docs/interstitial-chapter-design.md` — inserting an unplanned chapter ahead of the write frontier, splitting containment from provenance (`isolated` vs `generator`), the `generate-unrestricted`/paste-prose fill paths, the isolated-chapter finalize gate + summarize autofill, and in-place amendment of finalized canon. **Amends Appendix A rule 8** (its §2); drives tasks IC1–IC9.
...
- [x] PI3 — Authoring skill (external, `~/.claude/skills/novel-plan-forge/`): SKILL.md process, workspace templates, zero-dep `pack.mjs` targeting the §2 bundle (plan-import §7). Verify: packed example workspace imports cleanly against a local server. (skill since retired; in-app authoring is primary — plan import remains for legacy bundles)
...
- [ ] CK5 — Follow-ups: plan-import `facts` collection + brief `knowledgeContract`, `novel-plan-forge` skill update (skill since retired; in-app authoring is primary), web facts panel, `forRevision` sections, arc-planner reveal authoring (character-knowledge §8).
...
- [ ] RT1–RT11 — Transformative reforge mode. The task list with its per-task verification lives in `docs/reforge-transform-design.md` §10 and is ticked there; this line is done
      when every RT checkbox in that doc is ticked.
...
- [x] IC1 — Enum rename + isolated columns + migration: `content_generator` `'grok'` → `'unrestricted'` (`RENAME VALUE`, checked against a drop-and-recreate emission), `chapters.isolated`/`drafts.isolated` boolean backfilled from the old `generator = 'unrestricted'` value, `chapters.content_rating`/`drafts.content_rating` jsonb (interstitial-chapter §3.1–3.2). Verify: migration applies to the template DB, schema tests green, backfill leaves every existing row's containment behavior unchanged.
- [x] IC2 — Re-key containment: `indexing.service.ts`, `retrieval.service.ts`, `chapter-finalization.graph.ts`, `context-assembler.service.ts` (both sites), and `catalog.service.ts`'s outliner tag switch from `generator === 'grok'`/`'unrestricted'` to `isolated` (interstitial-chapter §4). Verify: a `human`-provenance chapter with `isolated: true` is excluded from index/retrieval/continuity exactly like an `unrestricted`-provenance one; an `unrestricted`-provenance chapter with `isolated: false` is not excluded.
- [x] IC3 — Remaining vendor renames: `/chapters/:n/generate-grok` → `/chapters/:n/generate-unrestricted` (controller/DTO/service rename), `continuity.prompt.ts`'s "grok-generated" system-prompt text reworded + prompt version bump + render golden regen, `ai-system-design.md` prose occurrences reworded per the rule-8 amendment, test names/fixtures updated (interstitial-chapter §5). Verify: prompt suite green on the bumped version, full suite green, no remaining `'grok'` string outside `models.ts`'s vendor model ids.
- [x] IC4 — Prerequisite defect fixes: `importDraft`'s `onConflictDoUpdate` set clause gains `generator` (and `isolated`/`contentRating`); `generateGrok`/`generateUnrestricted` loads the real `project` row instead of passing a `{ contentMode: 'unrestricted' }` stub into `modelRouter.structured` (interstitial-chapter §6). Verify: re-import over an existing draft keeps the new provenance; a project-level `config.models` override is honored on an unrestricted generation call.
- [x] IC5 — Feature schema + CHP codes: `briefs.write_mode` enum + `briefs.inserted_at`, `draft_revision_source` gains `'amended'`, `CHP_003`–`CHP_006` (interstitial-chapter §3.3–3.5). Verify: migration applies to the template DB, schema tests green.
- [x] IC6 — `ChapterInsertService.insertAfter`: frontier + in-flight-job guards, two-phase negative-parking shift across `briefs`/`drafts`/`chapters`/`chapter_images`/`continuity_proposals`, brief re-render + `contextRefs`/`knowledgeContract`/`canon_facts.reveal_chapter` rewrite, arc/volume growth, hand vs. planner slot-brief creation, `markDescendantDraftsStale`, `POST /chapters/:afterChapter/insert` (interstitial-chapter §7). Verify: shift/rewrite matrix + transactional insert e2e incl. the frontier and in-flight-job refusals.
- [x] IC7 — Filling the slot: `generate-unrestricted` sets `isolated`/`generator` on both drafts and chapters, `POST /drafts/:n/import` extended with `contentRating`/`state`/`isolated`, `GenerationService.generate`'s batch selection truncates (not skips) at an unfilled `write_mode = 'external'` chapter and resumes once it's `done` (interstitial-chapter §8). Verify: batch-truncation test asserting the accepted-cost shape (a 20-limit batch with an external slot at position 4 yields 3), paste-prose-isolated e2e.
- [x] IC8 — Finalize gate + summarize: `CHP_005` refusal when an isolated draft has no summary or state, `POST /chapters/:n/summarize` returning `{summary, state}` without persisting, `PUT /drafts/:n` DTO gains `state` (interstitial-chapter §9). Verify: gate matrix (isolated+empty refuses, isolated+filled passes, non-isolated always passes) + summarize idempotence.
- [x] IC9 — Amend: `POST /chapters/:n/amend`, `CHP_006` refusal on non-final chapters, write past `locked` without the `commitProse` `setWhere` guard, `draft_revisions` row with `source: 'amended'` attached to the existing draft without mutating it (`draftId` is `NOT NULL` with an FK, so the doc's `(projectId, chapter)` keying was never available; the revision number clears both counters so a second amend cannot collide), conditional re-embed (skipped when isolated), conditional republish (only on `contentHash` move), `extract-to-bible` follow-up flag in the response (interstitial-chapter §10). Verify: amend e2e on a locked chapter incl. the true no-op case — identical title/content/note, no republish — and the content-change case (republish scheduled). A note-only amend is **not** a no-op: `note` renders as `authorNote` and is inside `chapterContentHash`, so it republishes.
  - Follow-up: reader rating (cross-app) — `ReaderChapterPayload`/`chapter_publications.content_rating`, `chapterContentHash` extended (three-repo coordinated deploy + one `reconcile` run), web-novel-server's `published_chapters.content_rating` + `ChapterUpsertBody`, `web-novel-web` `api-types.gen.ts` regen, novel-vs-chapter rating invariant at publish (interstitial-chapter §11).
  - Follow-up: web UI (`novel-forge-web`) — insert action with shift preview, hand-brief vs. planner-intent dialog, `[unrestricted]` badges keyed on `isolated`, write-mode picker, rating picker, blocked-finalize state with the summarize button, amend action with the extract-to-bible follow-up link (interstitial-chapter §12).
  - Follow-up: `api-types.gen.ts` regen for `novel-forge-web` and `web-novel-web` from a booted server — CI's `gen-api-types --check` is RED until then; delete `novel-forge-web`'s temporary hand-written mirror `src/lib/apis/interstitial.api.ts` and its explicit re-exports in `src/lib/apis/index.ts` once the generated types carry those shapes.
  - Follow-up: run the DB-backed suites — ~894 specs skip without Postgres, including ALL of `tests/generation/chapter-insert.spec.ts` (the renumber) and most of `chapter-amend.spec.ts`; those two are the least-verified code in this feature and should run first. `web-novel-server`'s suite cannot run without Postgres at all (its `bunfig.toml` preloads `tests/setup.ts`, which unconditionally shells `create-template`), so its two new rating cases are unexecuted either way.
  - Follow-up: renumber-shift gaps found but not closed — `CHP_004` guards only `kind='generate'`, so an in-flight `finalize` or `extract` job (holding its chapter in `target`/`payload`) would act on the wrong chapter after a shift; `briefs.endingContract` is not chapter-rewritten on insert though `contextRefs` and `knowledgeContract` are, and its free-form fields can quote chapter numbers; `illustrations.subjectKey` stores a chapter number as TEXT, invisible to the integer shift rule, so an in-flight illustration session can save images against the wrong chapter; `entity_appearances.seen_chapters` (jsonb) is chapter-keyed and unshifted too — currently dead code, so a latent trap rather than a live bug, but wants a deny-list line before it isn't.
  - Follow-up: rating rollout gaps — `rating-invariant.ts` filters `!== undefined`, letting a null dimension through unnormalized before ranking; chapters published before the rating change keep their legacy digest, so a reader never learns their rating until someone republishes them explicitly; the web client's `isIsolated()` has no fallback, so a web build talking to an older server sees `undefined` and silently drops the containment badge for the rollout window.
  - Follow-up: the IC7 paste-prose-isolated e2e was never written — it needs the template DB.
