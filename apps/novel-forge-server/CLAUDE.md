...
**Current state:** the platform is built out — 61 of the 64 checklist tasks below are complete. The only open items are **CK5** (remaining knowledge follow-ups: web facts panel, `forRevision` sections, arc-planner reveal authoring, bible-audit spoiler check), **PB5** (the external `novel-forge-reader` service, which lives in its own repo), and **NI2** (the web upload UI for novel-import, a follow-up in the `novel-forge-web` repo). Treat the checklist below as a completed build log plus those three follow-ups — not a from-scratch plan.
...
- [ ] NI2 — Web UI (`novel-forge-web`): bundle upload screen — file picker for a hand-packed `novel-import` bundle, mode indicator, field-error list on a 422, progress tracking via the existing jobs endpoints. Separate follow-up in the web repo.
