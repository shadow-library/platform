...
- [ ] M1 — Project structure & dependencies (migration §10 Phase 1)
- [ ] M2 — PostgreSQL schema & Drizzle migrations, template-DB test setup (Phase 2, §6)
- [ ] M3 — Core domain modules: projects, chapters, entities, bible CRUD (Phase 3, §5/§7)
- [ ] M4 — Repository/persistence semantics: idempotent upserts, state transitions (Phase 4, §5.5)
- [ ] A1 — AI data model & migrations incl. checkpointer tables (design §10 A1, Appendix B)
- [ ] A2 — Prompt modules + Zod schemas, render goldens (A2, §5)
- [ ] A3 — Model router, telemetry callback, repair ladder, `AI_PROFILE` seam (A3, §5.4)
- [ ] A4 — ContextAssembler: catalog, ref resolution, budgets, packs (A4, §3)
- [ ] A5 — LlamaIndex retrieval: prose + lore indexes (A5, §7)
- [ ] A6 — Tool registry: 6 read-only tools, loop, audit (A6, §4)
- [ ] A7 — LangGraph workflows + WorkflowRunService (A7, §2)
- [ ] A8 — Human review API: feedback/approve/revisions/review-queue/runs endpoints (A8, §6)
