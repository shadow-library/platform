...
| P3  | Remove Ollama from the LLM path: the `ChatOllama` branch in `buildClient`, the two `ollama` LLM registry entries, the `provider !== 'ollama'` branch in `buildMessages`, the Ollama arms of `quota.ts`/`telemetry.handler.ts`, and the now-dead `CONSTRAINT_KEYWORDS` split if P1 left one. **Leave `EmbeddingService` and the `qwen3-embedding:8b` entry alone.** | High   | `modules/ai/{models,defaults,model-router.service,quota,telemetry.handler}.ts`, `bootstrap.ts` |
