...
**`POST /bible/audit`**. The `bible-audit` module (role `audit`, analytical, llm_cache-cacheable) receives the bible doc inventory (section/slug, revision, word count, first line), the **entity inventory** (records grouped by type), and `BIBLE_MANIFEST` — the single declaration of what a serialized web novel's bible needs, all slugs under the **existing** `bible_section` enum (no enum change). Output: `{findings[], changeSet}` — findings keyed `doc:<section>/<slug>` or `entity:<entityKey>`, and a change-set of `bible_document.upsert`/`remove` **and `entity.upsert`/`remove`** ops. Kind `bible_audit`, same apply/discard flow.

> **Amendment (bible manifest).** This section originally specified a documents-only manifest, `REQUIRED_BIBLE_DOCS`, living in `src/modules/refinement/`. It has been replaced by `BIBLE_MANIFEST` in `src/modules/bible/bible-manifest.ts`, which is read by the bible builder, this audit, and the readiness score alike. The change closes three defects that followed from having three separate notions of "the bible":
>
> 1. The builder's stage slugs (`project/foundation`, `world/world-power`, `ai/characters`, …) and the audit manifest's slugs (`project/premise`, `power/progression-ladder`, …) **never overlapped**, so a fully built bible audited as entirely missing. One manifest now names both; migration `0040_bible_manifest_slugs.sql` rewrites the legacy addresses in place, skipping any row whose target address is already taken.
> 2. No builder stage prompt ever asked for `entities`, and `BIBLE_STAGE_OUTPUT_SHAPE` makes the field conditional on being asked — so the builder produced documents and no records, while the Story Bible screen reads records only. Each manifest chapter now declares `materializes` + `minEntities`; the stage prompts state the requirement and a `postValidate` rejects a stage that returns none, so the repair ladder retries (Appendix A rule 5).
> 3. The audit itself could only emit document ops, so it could not repair the condition in (2). Its vocabulary now includes the entity ops.
>
> The manifest also carries `requiredTopics` per chapter, which the audit judges inside the body — this is how the original manifest's finer-grained entries (`reader-promise`, `pacing-tone`, `ending-vision`, `rules-and-limits`) survive without one document per concern. Section `ai` no longer holds the cast; `project/cast` does.
