...
- [ ] **RT5** — Plan: `reforge-plan` prompt (role `plan`) + schema, pure `validateTransformPlan`, plan draft/edit/approve service, `spanKey` carry-forward across revisions, plan endpoints. _Verify:_ validator matrix (gaps, overlaps, bad targets, drop-first, missing bridge) + revision carry-forward test.
