...
- [x] **RT7** — Span-transform graph: `reforge-transform-write` + `reforge-transform-judge` prompts & schemas, deterministic resurfaced-cut pre-scan, `span-transform.graph.ts`, exported pure `routeAfterTransformJudge`, `runSpanTransform`. _Verify:_ route matrix + mocked-router graph runs; a planted resurfaced cut is caught by the pre-scan alone.
      `loadSpan` derives the span and `indexInSpan` from the plan's own running target sum rather than trusting a caller-supplied ordinal, and refuses outright when the plan is not `approved` (hard rule 16). The judge's own `resurfaced_cut` issues are dropped on merge because the pre-scan already reports the same hits — keeping both would double every repair note. Seeded and discovered ledger entries are told apart by `createdAt <= plans.approvedAt`, which avoids a discriminator column; a chapter's own `cutDelta` binds from `outputChapter + 1`, since the chapter that made the cut is the one allowed to describe it.
- [x] **RT8** — Transform stage & endpoints: `transform` stage (plan/glossary verification, data-derived output selection, flag-and-continue), output/cut/manuscript endpoints, mode dispatch in `ReforgeService.status`/`renderManuscript`. _Verify:_ executor + controller e2e; a `chapter`-mode project's existing endpoints byte-identical to pre-RT1 responses.
      The chapter-mode goldens are byte-identical apart from one additive field: `mode` on `ReforgeResponse`. Nothing can dispatch on a mode
      it cannot read, so the alternative was inferring it from the presence of the `transform` block. `status` keeps chapter mode's exact
      shape and grows a `transform` block (plan header, output counts, ledger size) only in transform mode; that block **omits** `plan`
      rather than sending `null`, because the response serializer has no nullable-object form. The endpoint gate for `POST /transform` and
      `POST /outputs/:n` runs before enqueue, so an unapproved plan 400s (`REF_005`) instead of parking a job that cannot run. `§9`'s
