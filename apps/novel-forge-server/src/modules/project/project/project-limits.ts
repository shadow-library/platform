...
export async function assertUnderProjectCap(db: PrimaryDatabase, ownerId: bigint): Promise<void> {
  const cap = Config.get('projects.max-per-owner') ?? DEFAULT_PROJECT_CAP;
  if (cap <= 0) return;
  const count = await db.$count(schema.projects, eq(schema.projects.ownerId, ownerId));
