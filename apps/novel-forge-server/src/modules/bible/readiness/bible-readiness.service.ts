...
export class BibleReadinessService {
  ...
  async score(projectId: bigint): Promise<BibleReadinessReport> {
    ...
    const [docs, entities, facts] = await Promise.all([
      this.db.query.bibleDocuments.findMany({ where: eq(schema.bibleDocuments.projectId, projectId), columns: { section: true, slug: true, body: true } }),
