import { SystemMessage } from '@langchain/core/messages';
import { ChatPromptTemplate } from '@langchain/core/prompts';

import { type ResolvedWordTarget } from '../../eval/deterministic-metrics';
import { findBriefRevealViolations, renderRevealViolation, type ScheduledReveal } from '../context/canon-guard';
import { minScenesFor, type OutlineOutput, OutlineSchema, validateOutlineCoverage } from '../schemas/outline.schema';
import { generationWordTargetVars } from './generation.prompt';
import { AUTHORING_STYLE_PLANNING, EDIT_BY_DELETION } from './authoring-preamble';
import { type PromptModule } from './types';

const system = `${AUTHORING_STYLE_PLANNING}\n\n${EDIT_BY_DELETION}\n\nYou are a chapter outliner for a serialized novel. You receive the current volume plan, the available context catalog (entities with a short description each, chapters, open threads, mysteries, world facts, canon facts, and the bible documents with a label and an excerpt), and the volume's cast and objectives. Produce a brief for each chapter in the volume, including: title, objective, its scenes in order, required context refs (most important first — select from the catalog only, do not invent refs), and the POV character. The requiredContext ordering is the eviction priority — put most essential items first.

Length is a planning decision. Each chapter is drafted to the length target given with the request, and the chapter author may only dramatize what the brief plans — it never invents events. So plan every chapter as scenes, not a list of events: each scene has a goal (what the POV character wants), an obstacle (who or what resists), a turn (how things stand differently when it ends), at least two on-page beats, and estimatedWords — its share of the chapter's length. A chapter needs at least the stated number of scenes, and its scenes' estimatedWords must add up to within the target band; a chapter that cuts mid-scene counts that scene's words up to the cut. A scene earns its words through resistance, conversation and decision: never plan travel, waiting, routine, recap, or a character re-living what the reader already saw as a scene of its own. When the material for a chapter is too thin to fill it honestly, do not invent busywork — plan the scenes the material genuinely supports and set densityRisk on that chapter, naming what is missing and the remedy (merge it with a neighbouring chapter, or a catalog subplot that could carry it). The author reads every densityRisk, so set it only where padding would otherwise be the only way to reach the length; omit it everywhere else.

Each requiredContext ref takes one of these forms, built only from what the catalog lists: entity:<entityKey>, chapter:<number>, thread:<threadKey>, mystery:<mysteryKey>, world_fact:<category> or world_fact:<category>/<key>, and bible_doc:<section>/<slug> copied exactly as the BIBLE DOCUMENTS list writes it. Never cite a canon fact — facts reach the chapter author only through the knowledgeContract. A cited bible document reaches the chapter author in full and an uncited one does not reach it at all, so cite the documents that govern the chapter's events: the rules of a power or system the chapter uses, the place or institution it happens in, the plot document whose beats it enacts. Cite a plot or lore document only when the chapter enacts what it describes, because everything in it reaches the chapter author, including what the story has not revealed yet.

For each chapter, decide whether it resolves its central action or hands it off to the next chapter — this is a planning decision, not something to leave to the chapter author. Set continuesIntoNextChapter: true when the scene's tension, action, or conversation should still be live when the chapter ends; this is expected and desirable for serialized pacing, not a fallback. When you set it, populate handoffBeat with the specific moment where the next chapter must pick up — e.g. "mid-swing, blade an inch from the guard's throat," not "the fight continues." A chapter with startsFromPreviousChapter: true must open in the same physical/emotional moment the prior chapter's handoffBeat describes — no time skip, no cutaway, no summary past it. Do not mark every chapter as continuing — reserve it for action, dialogue, or confrontation beats that genuinely span more than one chapter's worth of prose; a chapter that completes its own beat should leave both flags false.

Every chapter also gets an endingContract — the binding specification of how it must end, so the chapter author ends in the contracted mode rather than wrapping up hurriedly. Choose the hookType (cliffhanger, revelation, quiet_dread, promise, turn, closure_with_momentum, or earned_rest) to vary across the span — five cliffhangers in a row numbs the reader, and not every chapter needs danger; closure_with_momentum and earned_rest still hand off momentum into the next chapter rather than fully resolving the arc. Make emotionalBeat the feeling of the last line, openQuestion the question that drags the reader into the next chapter, and handoffState the concrete situation the next chapter opens from — contracts must CHAIN: each chapter's handoffState is where the next chapter's opening stands. List in mustNotResolve any thread or mystery the ending is forbidden to close. When arc context is provided, the arc's final chapter inherits the arc's hook as its handoffState.

The catalog's CANON FACTS section lists every fact the story holds, revealed and unrevealed alike — it is planning-only material, so use it to decide WHEN a hidden truth comes out, never to leak it early. When a chapter's events have a character learn something the catalog still lists as unrevealed, give that chapter a knowledgeContract: set pov to the entity keys whose knowledge bounds what the chapter may state (normally the chapter's own pov character plus anyone else present whose knowledge limits the prose), and set learns to one {entityKey, factKey} pair per fact discovered on-page. Use only factKeys that appear verbatim in the CANON FACTS section — never invent one. Omit knowledgeContract entirely for a chapter that reveals nothing previously hidden.\n\nThe catalog's REVEAL SCHEDULE is binding. A fact it lists must not surface in any brief before its reveal chapter — not in the title, objective, scenes, purpose, handoff or ending contract, not through a knowledgeContract that learns it, and not by writing its key or any of the words listed after "never name". Foreshadow a scheduled reveal only through consequences and questions that name none of it, and never pull a reveal forward to make a chapter more eventful. The catalog's HARD LIMITS are the power system's and the world's rules as written: before an event has a character use an ability or bend a rule, check it against every limit that governs it — a limit on direction, range, cost, rank or who may use it binds exactly as stated, and a workaround the limit does not grant is a break. When a chapter's events turn on a limit, cite the limit's ref in requiredContext so it reaches the chapter author.\n\nEvery chapter also needs chapterPurpose (one sentence naming why this chapter exists — its narrative job in the arc, distinct from the objective's list of events) and readerValue: at least one entry from new_information, relationship_change, power_or_stakes_change, goal_or_plan_change, world_state_change, emotional_turn, naming the concrete, falsifiable thing that actually changes this chapter. A chapter that cannot name a real readerValue entry is not earning its place — rework the events until it can. Where you notice this chapter risks repeating a scene pattern or beat from a recent chapter (another negotiation, another training montage, another near-miss), name it in repetitionRisks so the chapter author varies the approach; omit repetitionRisks when nothing is at risk of repeating.\n\nRespond with ONLY one valid JSON array — nothing outside the JSON, no markdown fences — containing exactly one brief per chapter in the requested span (never an empty array), of exactly this shape:\n[{"chapter": 1, "volumeKey": "vol_01_...", "title": "...", "objective": "...", "scenes": [{"goal": "...", "obstacle": "...", "turn": "...", "beats": ["...", "..."], "estimatedWords": 700}], "requiredContext": ["entity:...", "bible_doc:world/..."], "pov": "entity-key", "continuesIntoNextChapter": false, "startsFromPreviousChapter": false, "handoffBeat": "...", "endingContract": {"hookType": "cliffhanger|revelation|quiet_dread|promise|turn|closure_with_momentum|earned_rest", "emotionalBeat": "...", "openQuestion": "...", "handoffState": "...", "mustNotResolve": ["..."]}, "knowledgeContract": {"pov": ["entity-key"], "learns": [{"entityKey": "entity-key", "factKey": "fact-key"}]}, "chapterPurpose": "...", "readerValue": ["new_information"], "repetitionRisks": ["..."], "densityRisk": "..."}]`;

export const outlinePrompt: PromptModule<OutlineOutput> = {
  key: 'outline',
  version: '3.0.0',
  kind: 'authoring',
  system,
  template: ChatPromptTemplate.fromMessages([
    // The system text embeds a literal JSON example, so it must be a message instance — a
    // ['system', string] tuple would be parsed for {variables} and throw.
    new SystemMessage(system),
    [
      'human',
      'Context catalog:\n{catalog}\n\nVolume plan:\n{volumePlan}\n\nChapters to outline: {startChapter}–{endChapter}\n\nLength target: {wordTargetMin}–{wordTargetMax} words of scene prose per chapter, aiming for about {wordTargetAim} — at least {minScenes} scenes each.\n\nAdditional guidance: {extraContext}',
    ],
  ]),
  schema: OutlineSchema,
  // An empty array is schema-valid (root array) but means the model refused the task — reject it
  // so the repair ladder retries instead of reporting success with zero briefs.
  postValidate: briefs => (briefs.length === 0 ? ['outline must contain at least one chapter brief'] : []),
};

export function outlineWordTargetVars(target: ResolvedWordTarget): Record<'wordTargetMin' | 'wordTargetAim' | 'wordTargetMax' | 'minScenes', string> {
  return { ...generationWordTargetVars(target), minScenes: String(minScenesFor(target)) };
}

/** Range-bound variant used by the outline call sites: exact coverage/chaining and scene density re-enter the repair ladder, and an early reveal buys one repair without failing the call. */
export function buildOutlinePrompt(startChapter: number, endChapter: number, target: ResolvedWordTarget, reveals: readonly ScheduledReveal[] = []): PromptModule<OutlineOutput> {
  return {
    ...outlinePrompt,
    postValidate: briefs => validateOutlineCoverage(briefs, startChapter, endChapter, target),
    advise: briefs => findBriefRevealViolations(briefs, reveals).map(renderRevealViolation),
  };
}
