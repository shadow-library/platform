...
/**
 * Scope-bound variant: the repair ladder forces the model back inside the scope's op allowlist. A prose op the author did not
 * ask for is advisory rather than blocking, so a model that insists costs one repair and then loses the op, never the turn.
 */
