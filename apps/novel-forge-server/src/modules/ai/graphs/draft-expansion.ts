...
import { countWords, WORD_TARGET_AIM, WORD_TARGET_MIN } from '../../eval/deterministic-metrics';
...
import { WORD_COUNT_HARD_MAX } from './mechanical-check';
...
export async function expandShortDraft(
  ...
  while (words < WORD_TARGET_MIN && passes < maxPasses) {
    passes++;
    const expandCtx: TelemetryContext = { ...ctx, node: `${ctx.node ?? 'draft'}:expand`, promptKey: prompt.key, promptVersion: prompt.version };
    const vars = { ...input, draftBody: body, draftWords: words, minWords: WORD_TARGET_MIN, aimWords: WORD_TARGET_AIM, missingWords: WORD_TARGET_AIM - words };
    ...
    if (!expanded || expandedWords <= words || expandedWords > WORD_COUNT_HARD_MAX) break;
    body = expanded.body;
    words = expandedWords;
  }
