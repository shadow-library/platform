...
/**
 * The app-facing HTTP client: the same chainable, thenable builder every `*.api.ts` file has always used,
 * now a thin facade over the `@shadow-library/web` server-function transport instead of a hand-rolled
 * isomorphic `fetch`. Paths are relative to the versioned API (`/dashboard/stats`, not `/api/v1/dashboard/stats`).
 */
