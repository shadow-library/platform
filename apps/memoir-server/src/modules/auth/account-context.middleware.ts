 ...
 */
const ALLOW_DURING_DELETION: unique symbol = Symbol('memoir:allow-during-deletion');
...
export class AccountContextMiddleware {
  ...
  cacheKey(metadata: HandlerMetadata): string {
    return `memoir:account-context:${String(metadata.method)}:${String(metadata.path)}`;
  }
