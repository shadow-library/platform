...
const baseConnectionString = process.env['DATABASE_POSTGRES_URL'] ?? 'postgresql://postgres:postgres@localhost:7070/memoir';
const baseUrl = baseConnectionString.replace(/\/[^/]*$/, '');
const templateDbName = process.env['POSTGRES_TEMPLATE_DB_NAME'] ?? 'memoir_template';
