...
- Pulse is an OAuth2 **resource server** standardised on `@shadow-library/auth/module`: `AuthModule.forRoot` provides the `AuthClient` and the shared `AuthGuard` preValidation hook, which verifies EdDSA bearer tokens offline against the identity JWKS and enforces access per route (401 `IAM_001` / 403 `IAM_002`).
