...
Migration via `bun scripts/db.ts apps/identity-server generate` (from the repo root); `bun scripts/check-migrations.ts apps/identity-server` must be clean; the test template DB must rebuild.
