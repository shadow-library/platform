...
### T-905 — SCIM group → app & role mapping · L — **deferred (design dependency only)**

- Groups today are provisioning structure with no authorization semantics. Once M8a lands, map IdP groups to `organisation_applications` assignments and `ORGANISATION`/user role grants. Needs its own task spec; M8a's primitives were shaped so this is additive.
