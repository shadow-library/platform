...
| **D-2**  | Service-to-service authentication uses **OAuth 2.0 client credentials** with short-lived JWT access tokens issued by this service. Service accounts are OAuth clients (`kind = SERVICE`). Resource servers never verify a static API key, and there is no mTLS mesh. The one long-lived credential is an organisation bot key (§7.4). It is never an authorization input itself: a first-party resource server exchanges it at identity (RFC 8693, D-22) for a short-lived, org-bound, audience-restricted JWT, and verifies only that token.                                                                                                                                                                                                                                                | One token format and one verification path for human and machine calls; SDK handles acquisition/caching. mTLS rejected: no mesh infrastructure, no threat-model justification yet.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| **D-23** | **Organisation bot keys** (§7.4) are the one exception to D-2's "no static API keys": an admin-issued, long-lived `sl_bot_…` secret is exchanged for the short-lived JWT of D-2 rather than requiring every integrator to implement RFC 7523 client assertion or dynamic client registration.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | Trades integrator ergonomics for a resource server briefly holding a raw key in process memory between receipt and exchange. That exposure is bounded on every axis: the audience is forced to the exchanging client's own resource, the exchange cache is keyed by `sha256(key)` rather than the raw secret, and the key is never logged. Narrower and shorter-lived than a static API key a resource server would otherwise verify directly.                                                                                                                                                                                                                                                                               |
...
- **Service accounts** — modelled as OAuth clients with `kind = SERVICE`, scoped to an organisation (platform services live in the platform organisation). **Bots** (§7.4) are organisation-owned service accounts: the client stays the registry entry and PDP principal, and a `bots` row holds only what a client cannot express (handle, lifecycle status, IP allowlist, rate limit, creator). Bot keys (`bot_keys`) are exchange-only credentials, distinct from client secrets. One client registry and one audit trail remain.
...
An organisation admin creates a **bot**: a non-human, organisation-owned service account that acts like a user holding only the permissions it was granted, and owns the records it creates (D-2, D-23).

- **Principal.** An exchanged bot token carries `token_type: "bot"`, `sub` = the bot's client id, `bot_id`, `bot_key_id`, a mandatory `org`, and `rl` (the bot's rate limit); the PDP still resolves it as `SERVICE_ACCOUNT` (§11). TTL is fixed at 300 seconds, short enough that revocation and suspension propagate within one exchange interval.
- **Credential.** The one long-lived secret is a bot key: `sl_bot_<keyId: 22 base62>_<secret: 43 base62>_<crc32: 6 base62>`. Only `sha256(secret)` is stored; the checksum lets secret scanners and the SDK reject a malformed key without a network call.
- **Exchange.** A bot key is never an authorization input itself (D-2): a first-party resource server exchanges it at `/oauth2/token` (RFC 8693, D-22) for the token above. The audience is forced to the exchanging client's own resource, so a compromised resource server can replay a key only into its own API, and only first-party clients may exchange one. A bot token is refused as a D-22 subject token, so delegation stays single-hop. Revocation and suspension reach every replica within 60 seconds; a failed exchange is negative-cached for 10 seconds.
- **Grants.** A bot's permissions are `role_assignments` rows with `principal_type = SERVICE_ACCOUNT`, exactly like any other service account. An admin may grant only a role that is both **bot-eligible** in its application's catalog and **currently held by the granting admin** — no escalation through a bot. A grant persists when its granter later loses the permission; it is flagged for review rather than silently revoked, so a demotion never breaks production automation.
- **Limits.** Bots exist only for `TEAM` organisations, capped at 25 per organisation. A bot may hold at most 2 active keys, and every key requires an expiry of at most 365 days.
- **Deny by default.** `@shadow-library/auth` denies a bot principal on every route unless it carries `@BotPermission('<permission>')`; `@Authenticated()` alone never admits a bot. This is enforced once, in the SDK, rather than per application.
- **Identity's own organisation API.** Identity does not exchange keys for itself: its `AccessGuard` accepts `Authorization: Bearer sl_bot_…` directly, only on a route that opts in with `@Auth({ bot: '<permission>' })`. The key passes the same checks as an exchange (hash, expiry, revocation, bot and organisation `ACTIVE`, IP allowlist against the connection address), the route's `:organisationId` must be the bot's organisation, and the PDP must permit the permission for the bot's `SERVICE_ACCOUNT` principal scoped to the `shadow-identity` application, so a same-named permission in another application grants nothing. A route that is also `elevated`, `permission` or `service` never admits a bot. The v1 routes are listing members, invitations and domains, changing a member's status, and sending or revoking invitations. Within them a bot reaches no further than `MEMBER`: it changes the status of `MEMBER`s only, invites only as `MEMBER`, and revokes or replaces only `MEMBER` invitations. It never acts on an `OWNER` or `ADMIN`, changes roles, removes members, or reaches an elevated route. Its actions are audited with actor type `SERVICE_ACCOUNT`.
- **Rate limits.** A key may be exchanged 60 times a minute, and a bot may make `rate_limit_per_minute` requests a minute to identity's own API. Both are product quotas and stay enforced when the `RATE_LIMIT_ENABLED` abuse-control kill switch is off. Successful exchanges are audited at most once an hour per key and refusals once per key per reason per five minutes, because every audit write takes the organisation's hash-chain lock; refusal logs are sampled per minute.
- **Deletion.** A bot owns records in other applications, so deleting one is a handover rather than a delete. `DELETE /bots/:botId` takes an ACTIVE member of the bot's own organisation and the bot's handle typed back, revokes every key, moves the bot to `DELETING`, deactivates its client, bumps the principal's authz version and answers `202`. One `bot_ownership_transfers` row per **bot-aware application** is enqueued in the same transaction; the worker drains them with per-row backoff and marks the bot `DELETED` only once every row is `DONE`. An application is bot-aware when it exposes `<application>:bots:manage` on its own `api://<application>` resource **and** identity's outbound service client holds that scope — the same pair that admits identity to its `/internal/bots/*` routes, so the set is read from the grants rather than listed anywhere. The `bots` row survives as a tombstone: the handle stays reserved because records elsewhere still render it. A transfer that exhausts its attempt budget stops being retried and surfaces on the bot (`deletion.stalled`) for an administrator to retry; deletion never completes silently half-done. Two consequences of the split between the request and the handover are deliberate and unfixable from identity alone. **The recipient is validated when deletion is requested, not when each transfer lands:** they must be an ACTIVE member with an ACTIVE account at that moment, but an application confirming hours later still assigns the records to them even if they have since left the organisation or been suspended — the alternative, re-validating per application, would strand a deletion halfway through on a membership change. **An application that stops being bot-aware after its row is enqueued can never satisfy that row:** the worker has no scope left to call it with, so the row fails until it exhausts and the bot stays `DELETING` until an operator restores the grant or the row is removed. Both are visible in the bot's deletion state rather than silent.
- **IP allowlist trust.** An exchange evaluates the allowlist against the `client_ip` the first-party resource server forwards, which that server takes from its own `request.ip`. The allowlist is therefore only as strong as each first-party application's trusted-proxy configuration (`APP_TRUST_PROXY` set to the ingress's trusted CIDR list): an application that sets it to `true`, trusting `X-Forwarded-For` from any hop, lets a caller spoof an allowlisted address.
