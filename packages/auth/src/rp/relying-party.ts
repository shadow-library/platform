...
export class RelyingParty {
  ...
  private async requestTokens(body: Record<string, string>): Promise<TokenSet> {
    ...
    if (!payload.access_token || typeof payload.expires_in !== 'number') throw this.logged(AuthErrorCode.EXCHANGE_FAILED.create({ reason: 'malformed token endpoint response' }));
    return {
