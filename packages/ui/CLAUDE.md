...
- The **build** (`bun scripts/build.ts packages/ui` from the repo root, inferred `type: component`) bundles ESM with Rollup + PostCSS, extracts all CSS Modules into one `dist/styles.css`, emits type declarations, and injects `'use client'` banners — the subpaths come from this package's `exports` map, and the two non-inferable knobs (`css` / `alias`) live under the `"shadow"` key in `package.json`.
...
| Releases            | Retired — this package is private (`UNLICENSED`) and not published; root Husky still enforces the commit-message format (`.husky/commit-msg` → `commitlint` against the root `commitlint.config.ts`)                                                                                                                                                                                                        |

**Do not introduce Tailwind CSS, CSS-in-JS runtimes, or any other styling system.** Components style exclusively through CSS Modules; the library also ships one hand-authored, token-backed, **unprefixed** global utility layer (`src/styles/utilities.css`) for consumers' own layout markup — see _Styling Guidelines_ rule 4.
...
7. **Avoid unnecessary nesting.** Prefer flat selectors; express variants/sizes/states with `data-*` attribute selectors on `.root` (see _Variant/Size implementation_).
...
Every component must be keyboard-operable, expose correct roles/ARIA, manage focus, and meet WCAG 2.1 AA. See the _Accessibility Checklist_.
...
.root[data-size='sm'] {
  height: var(--sh-control-height-sm);
}
.root[data-size='lg'] {
  height: var(--sh-control-height-lg);
}
.root:disabled {
  opacity: var(--sh-disabled-opacity);
  cursor: not-allowed;
}
```
...
| **Page layout**             | `--sh-page-max`                                                                                                                      | The platform reading column, mirroring the `PAGE_WIDTH` export that is `Shell`'s `contentWidth` default. For apps that take the content region whole (`contentPadding="none"`) and must centre their own column; pair with `--sh-shell-gutter-*` for matching gutters. |
...
2. **Write the component** `ComponentName.tsx` (+ `ComponentName.types.ts`) — read the corresponding design file in the _Shadow UI Design System_ project first; consume tokens; follow the variant/size/state pattern.
...
```bash
# From the REPO ROOT — this workspace has no verify/build scripts of its own:
bun scripts/verify.ts packages/ui   # prettier + eslint + tsc + vitest unit project (add --fix to auto-format/lint)
bun scripts/build.ts packages/ui    # type: component → rollup + postcss CSS Modules + tsc declarations + synthesized dist/package.json

# From packages/ui (its own scripts):
bun run build-storybook             # optional but recommended for new components
```

Commit as a **single logical unit** using Conventional Commits (`feat: add button component`). Husky runs `bun scripts/verify.ts packages/ui --fast` pre-commit; `commitlint` enforces the message format.
