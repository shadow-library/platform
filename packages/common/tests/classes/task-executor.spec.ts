 ...
 */
import { InternalError, TaskExecutor } from '@shadow-library/common';
...
describe('TaskExecutor', () => {
  ...
  it('retries once then succeeds', async () => {
    ...
    const result = await TaskExecutor.create(fn).delay(10).backoff(2).retry(2).onRetry(onRetry).execute();
  ...
  it('fails after all retries', async () => {
    ...
    await expect(TaskExecutor.create(fn).delay(10).retry(2).execute()).rejects.toThrow('fail');
    expect(fn).toHaveBeenCalledTimes(2);
  });
  ...
  it('executes rollback after success', async () => {
    ...
    const task = TaskExecutor.create(fn).rollback(rollback);
    await task.execute();
  ...
  it('throws error if rollback is called with no result', async () => {
    const fn = jest.fn().mockRejectedValue(new Error('fail'));
    const rollback = jest.fn();
    const task = TaskExecutor.create(fn).delay(10).rollback(rollback);
