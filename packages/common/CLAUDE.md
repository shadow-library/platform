...
- **Config keys.** Dot-delimited `<domain>.<area>.<name>`; only `[a-z.-]` (lowercase, dots, hyphens — no uppercase/underscores/digits). Env var is derived (`.`/`-` → `_`, uppercased): `db.pool.max` ⇄ `DB_POOL_MAX`. Declare keys+types via `interface X extends ConfigRecords`. Any prefix is subscribable.
